...
console.log(getEmptyArray(10, 0))
console.log(getEmptyArray(5, null))

// hoisting - поднятие, всплытие

// function declaration
function getEmptyArray(arrayLength, defaultValue) {
    const array = [];
...
// function expression
const getFirstN = function (array, count) {
    return array.slice(0, count);
}

console.log(getFirstN(getEmptyArray(5, null), 3));

// ECMAScript 5 - old JavaScript
...
console.log(calculator(5, 10, function (number1, number2) {
...
const mappedAncestry = ancestryFile
    .map(person => ({
        name: person.name,
        age: person.died - person.born,
        sex: convertSex(person.sex)
    }))
    .filter(person => person.age > 75);

const ancestryHtml = mappedAncestry.map(person => {
    const sexClass = person.sex === "муж." ? "male" : "female";
    // if(person.sex === "муж."){
    //     sexClass = "male";
    // }else{
    //     sexClass = "female";
    // }
    return `<div class="list-item ${sexClass}">
                ${person.name} (${person.age})
            </div>`;
}).join("");

const listContainer = document.querySelector(".list");
listContainer.innerHTML = ancestryHtml;
